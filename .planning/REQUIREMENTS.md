...
- [ ] **STEP-01**: state.yml schema includes optional `current_step` field (`skill`, `step`, `label`)
- [ ] **STEP-02**: state.yml.template documents the `current_step` field
...
- [ ] **STEP-09**: Status skill displays current step ("scope: step 5 of 9 — Adaptive Refinement") when `current_step` field exists; gracefully omits when absent
...
- [ ] **HAND-03**: G3 gate S4 criterion validates correctly with HANDOFF.md present
- [ ] **HAND-04**: PROJECT.md updated to reflect HANDOFF.md as officially supported (moved from Out of Scope)

### Gate Enforcement

- [ ] **GATE-01**: G2 (research) validates every DA readiness criterion marked MET in SYNTHESIS.md — MUST criterion
- [ ] **GATE-02**: G3 (design) validates evidence citations address DA-specific readiness criteria — MUST criterion
